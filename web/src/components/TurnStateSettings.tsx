...
export function TurnStateSettings() {
  ...
  function action(action: "probe" | "harvest" | "clear" | "stop" | "resume") {
    if (!entry.trim() || !model.trim() || !window.confirm(action === "probe" || action === "harvest" ? t("turnStateCostConfirm") : t("turnStateScopeConfirm"))) return;
  ...
  const fieldLabels: Record<string, string> = {
    ...
    "ticket.enabled": t("turnStateTicketEnabled"), "ticket.target_length": t("turnStateTicketTarget"),
    "ticket.harvest_proxy_url": t("turnStateTicketHarvestProxy"), "ticket.fallback": t("turnStateTicketFallback"),
    "ticket.revoke_after_signals": t("turnStateTicketSignals"),
  ...
  const optionLabels: Record<string, Record<string, string>> = {
    ...
    "ticket.fallback": { open: t("turnStateTicketFallbackOpen"), closed: t("turnStateTicketFallbackClosed") },
  };
  ...
  const ticketConfig = config?.ticket ?? ticketDefaults;
  const updateTicket = (key: keyof TurnStateConfig["ticket"], value: unknown) =>
    setConfig((current) => current ? { ...current, ticket: { ...ticketDefaults, ...current.ticket, [key]: value } } : current);
  const ticketNumber = (key: "target_length" | "revoke_after_signals", hint: "turnStateTicketTarget" | "turnStateTicketSignals") =>
    <SettingItemControl label={fieldLabels[`ticket.${key}`]} hint={t(hint)} saving={busy} saved={false} isDirty={false} requiresRestart={false}>
      <input class={inputCls} type="number" min="1" value={ticketConfig[key]} disabled={busy} onInput={e => updateTicket(key, Number(e.currentTarget.value))} />
    </SettingItemControl>;
  return <section class="bg-white dark:bg-card-dark border border-gray-200 dark:border-border-dark rounded-xl shadow-sm overflow-hidden" aria-label={t("turnStateTitle")}>
    ...
    <div class="px-5 py-2">
      ...
      {config && <>
        ...
        <p class="text-xs text-slate-500 dark:text-text-dim py-2">{t("turnStateTicketSection")}</p>
        <SettingItemControl label={fieldLabels["ticket.enabled"]} hint={t("turnStateTicketEnabledHint")} saving={busy} saved={false} isDirty={false} requiresRestart={false}>
          <input type="checkbox" checked={ticketConfig.enabled} disabled={busy} onChange={e => updateTicket("enabled", e.currentTarget.checked)} class="w-4 h-4 rounded border-gray-300 dark:border-border-dark text-primary focus:ring-primary cursor-pointer" />
        </SettingItemControl>
        <SettingItemControl label={fieldLabels["ticket.harvest_proxy_url"]} hint={t("turnStateTicketHarvestProxyHint")} saving={busy} saved={false} isDirty={false} requiresRestart={false}>
          <input class={inputCls} type="text" maxLength={512} placeholder="http://host:port" value={ticketConfig.harvest_proxy_url ?? ""} disabled={busy} onInput={e => updateTicket("harvest_proxy_url", e.currentTarget.value || null)} />
        </SettingItemControl>
        {ticketNumber("target_length", "turnStateTicketTarget")}
        {ticketNumber("revoke_after_signals", "turnStateTicketSignals")}
        <SettingItemControl label={fieldLabels["ticket.fallback"]} hint={t("turnStateTicketFallbackHint")} saving={busy} saved={false} isDirty={false} requiresRestart={false}>
          <select class={inputCls} value={ticketConfig.fallback} disabled={busy} onChange={e => updateTicket("fallback", e.currentTarget.value)}>{choices.ticketFallback.map(value => <option value={value}>{optionLabels["ticket.fallback"][value]}</option>)}</select>
        </SettingItemControl>
        ...
        <details class="border-t border-gray-100 dark:border-border-dark py-3"><summary class="text-xs font-semibold cursor-pointer">{t("turnStateAdvanced")}</summary>
          ...
          <div class="flex flex-wrap gap-3"><input class={inputCls} aria-label={t("turnStateAccountId")} placeholder={t("turnStateAccountId")} value={entry} maxLength={256} onInput={e => setEntry(e.currentTarget.value)} /><input class={inputCls} aria-label={t("turnStateModel")} placeholder={t("turnStateModel")} value={model} maxLength={256} onInput={e => setModel(e.currentTarget.value)} />{(["probe", "harvest", "clear", "stop", "resume"] as const).map(name => <button class="px-3 py-2 text-xs rounded-lg border border-gray-200 dark:border-border-dark disabled:opacity-50" disabled={busy || !entry.trim() || !model.trim() || (name === "probe" && (!overview?.config.active_enabled || !overview.config.enabled || overview.config.mode === "off")) || (name === "harvest" && (!(overview?.config.ticket ?? ticketDefaults).enabled || (overview?.config.ticket ?? ticketDefaults).harvest_proxy_url === null))} onClick={() => action(name)}>{name === "probe" ? t("turnStateProbe") : name === "harvest" ? t("turnStateHarvest") : name === "clear" ? t("turnStateClear") : name === "stop" ? t("turnStatePause") : t("turnStateResume")}</button>)}</div>
        </details>
      </>}
    </div>
  </section>;
